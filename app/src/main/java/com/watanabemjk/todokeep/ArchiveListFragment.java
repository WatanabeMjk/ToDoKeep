...
public class ArchiveListFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = new RecyclerViewAdapter(getActivity(),noteList,"ArchiveListFragment");
        ...
        SwipeableRecyclerViewTouchListener swipeTouchListener = new SwipeableRecyclerViewTouchListener(mRecyclerView, new SwipeableRecyclerViewTouchListener.SwipeListener() {
            ...
            public void onDismissedBySwipeLeft(RecyclerView recyclerView, int[] reverseSortedPositions) {
                ...
                for (int position : reverseSortedPositions) {
                    ...
                    TrashDatabaseAdapter.saveNote(removeNote.getYear(), removeNote.getMonth(), removeNote.getDay(), removeNote.getHour(), removeNote.getMinute(), removeNote.getDaysOfWeek(), removeNote.getTitle(), removeNote.getDetail(), removeNote.getBackColor());
                    final int noteId = removeNote.getId();
                    ArchiveDatabaseAdapter.deleteNote(noteId);
                }
    ...
    protected static void loadNote(){
        noteList.clear();
        ArchiveDatabaseAdapter.open();
        Cursor c = ArchiveDatabaseAdapter.getAllNotes();
        if(c.moveToFirst()){
            do{
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        switch (item.getItemId()){
            case R.id.action_settings:
                Log.d("ArchiveListFragment"," / onOptionItemSelected" + 123);
                        ...
                        .setPositiveButton(
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ArchiveDatabaseAdapter.open();
                                        ArchiveDatabaseAdapter.deleteAllNotes();
                                        loadNote();
                                        ArchiveDatabaseAdapter.close();
                                        ArchiveListFragment.loadNote();
                                    }
                                }
                        )
